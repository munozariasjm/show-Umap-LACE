...
def call_streamlit():
    ...
    st.markdown('LACE Embedding')

    return st


@st.cache(persist=True)
def load_data(sample_size=12_000, seed=0):
    df = pd.read_csv('data/lace_embedding.csv').iloc[12_000:].reset_index(drop=True)
